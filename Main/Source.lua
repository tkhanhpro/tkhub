if (game:GetService("CoreGui")):FindFirstChild("TK-Hub") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
    pcall(function()
        (game:GetService("CoreGui")).TKHub:Destroy();
        (game:GetService("CoreGui")).ScreenGui:Destroy();
    end)
end;
_G.Primary = Color3.fromRGB(120, 120, 120); -- Slightly brighter primary
_G.Dark = Color3.fromRGB(18, 18, 22); -- Darker dark
_G.Third = Color3.fromRGB(255, 50, 50); -- More vibrant red
_G.Accent = Color3.fromRGB(0, 200, 255); -- New accent color for highlights
...
ImageButton.MouseButton1Click:Connect(function()
    pcall(function()
        (game.CoreGui:FindFirstChild("TK-Hub")).Enabled = not (game.CoreGui:FindFirstChild("TK-Hub")).Enabled;
    end)
end);
...
function Update:Notify(desc)
    ...
    UIGradient.Color = ColorSequence.new(_G.Dark, _G.Third);
    UIGradient.Parent = Frame;
    OutlineFrame:TweenPosition(UDim2.new(0.5, 0, 0.1 + (#NotificationList) * 0.1, 0), "Out", "Quad", 0.4, true);
    table.insert(NotificationList, {OutlineFrame, Title.Text});
end;
...
(getgenv()).LoadConfig = function()
    if readfile and writefile and isfile and isfolder then
        ...
        if not isfolder("TK-Hub/lib") then
            makefolder("TK-Hub/lib/");
        end;
        if not isfile(("TK-Hub/lib/" .. game.Players.LocalPlayer.Name .. ".json")) then
            writefile("TK-Hub/lib/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(SettingsLib));
        else
            local Decode = (game:GetService("HttpService")):JSONDecode(readfile("TK-Hub/lib/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveConfig = function()
    if readfile and writefile and isfile and isfolder then
        if not isfile(("TK-Hub/lib/" .. game.Players.LocalPlayer.Name .. ".json")) then
            ...
            writefile("TK-Hub/lib/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
        end;
    else
        return warn("Status : Undetected Executor");
    end;
...
function Update:Window(Config)
    ...
    local TKHub = Instance.new("ScreenGui");
    TKHub.Name = "TK-Hub";
    TKHub.Parent = game.CoreGui;
    TKHub.DisplayOrder = 999;
    local OutlineMain = Instance.new("Frame");
    OutlineMain.Name = "OutlineMain";
    OutlineMain.Parent = TKHub;
    ...
    CreateCheckbox("Lưu Settings", SettingsLib.SaveSettings, function(state)
    ...
    CreateButton("Reset Cấu Hình", function()
        if isfolder("TK-Hub") then
            delfolder("TK-Hub");
        end;
